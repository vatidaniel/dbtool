...
import io.github.vatidaniel.dataaccess.SqlDialect;
...
public class StandardSqlDdlExecutor extends DdlQueryConstants implements DdlExecutor {
    ...
    protected ColumnMetadata getColumnMetadata(String columnName) {
        ...
        return collectionNullSafe(tableMetadata.getColumnsMetadata()).stream()
            .filter(c -> Optional.ofNullable(c.getName()).filter(columnName::equals).isPresent())
            .findFirst()
    ...
    /** Generate a deterministic constraint name for adding that don't carry an explicit name. */
